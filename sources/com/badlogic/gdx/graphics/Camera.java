...
public abstract class Camera {
    ...
    public Vector3 unproject(Vector3 screenCoords, float viewportX, float viewportY, float viewportWidth2, float viewportHeight2) {
        float x = screenCoords.x;
        float y = screenCoords.y;
        screenCoords.x = (((x - viewportX) * 2.0f) / viewportWidth2) - 1.0f;
        screenCoords.y = (((((((float) Gdx.graphics.getHeight()) - y) - 1.0f) - viewportY) * 2.0f) / viewportHeight2) - 1.0f;
    ...
    public Vector3 project(Vector3 worldCoords, float viewportX, float viewportY, float viewportWidth2, float viewportHeight2) {
        worldCoords.prj(this.combined);
        worldCoords.x = (((worldCoords.x + 1.0f) * viewportWidth2) / 2.0f) + viewportX;
        worldCoords.y = (((worldCoords.y + 1.0f) * viewportHeight2) / 2.0f) + viewportY;
        worldCoords.z = (worldCoords.z + 1.0f) / 2.0f;
        return worldCoords;
    }

    public Ray getPickRay(float screenX, float screenY, float viewportX, float viewportY, float viewportWidth2, float viewportHeight2) {
        float f = viewportX;
        float f2 = viewportY;
        float f3 = viewportWidth2;
        float f4 = viewportHeight2;
