...
public class ObjectFloatMap<K> implements Iterable<Entry<K>> {
    ...
    public float removeStash(K key, float defaultValue) {
        K[] keyTable2 = this.keyTable;
        int i = this.capacity;
        int n = this.stashSize + i;
        while (i < n) {
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < n; i++) {
            K key = keyTable2[i];
            if (key != null) {
                h = h + (key.hashCode() * 31) + Float.floatToIntBits(valueTable2[i]);
            }
        }
        return h;
    }
